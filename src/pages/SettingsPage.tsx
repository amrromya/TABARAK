...
function getFeatures(): { maintenance: boolean; attendance: boolean; dark_mode: boolean; language: boolean; sync: boolean; branches: boolean; attendance_url: boolean } {
  try {
    const raw = localStorage.getItem(FEATURES_KEY);
    if (raw) return { maintenance: false, attendance: false, dark_mode: false, language: false, sync: false, branches: false, attendance_url: false, ...JSON.parse(raw) };
  } catch {}
  return { maintenance: false, attendance: false, dark_mode: false, language: false, sync: false, branches: false, attendance_url: false };
}

function saveFeatures(f: { maintenance: boolean; attendance: boolean; dark_mode: boolean; language: boolean; sync: boolean; branches: boolean; attendance_url: boolean }) {
...
const SECTIONS: { key: SectionKey; icon: string; title: string; color: string; gradient: string; password?: string }[] = [
  ...
  { key: "sync", icon: "☁️", title: "syncSettings", color: "#8b5cf6", gradient: "linear-gradient(135deg, #8b5cf6, #7c3aed)", password: "5506" },
  { key: "branches", icon: "🏬", title: "branches", color: "#f59e0b", gradient: "linear-gradient(135deg, #f59e0b, #d97706)", password: "5506" },
  { key: "notifications", icon: "🔔", title: "attendanceNotifications", color: "#ec4899", gradient: "linear-gradient(135deg, #ec4899, #db2777)" },
  { key: "attendance_url", icon: "📍", title: "attendanceUrlLabel", color: "#14b8a6", gradient: "linear-gradient(135deg, #14b8a6, #0d9488)", password: "5506" },
...
export function SettingsPage() {
  ...
  const toggleFeature = async (key: "maintenance" | "attendance" | "dark_mode" | "language" | "sync" | "branches" | "attendance_url") => {
