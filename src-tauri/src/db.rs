...
fn migrate(conn: &Connection) -> Result<(), String> {
    ...
    // ترحيل كلمة مرور القسم إلى bcrypt
    let has_hash = conn.query_row(
        "SELECT COUNT(*) FROM settings WHERE key = 'section_password_hash'",
        [],
        |r| r.get::<_, i64>(0),
    ).unwrap_or(0);
    if has_hash == 0 {
        // إذا كانت كلمة المرور النصية القديمة موجودة، حوّلها
        if let Ok(stored) = conn.query_row(
            ...
            if !stored.is_empty() {
                if let Ok(hash) = bcrypt::hash(&stored, bcrypt::DEFAULT_COST) {
                    let _ = conn.execute(
            ...
            // لا توجد كلمة مرور — اضبط الافتراضية 5506
        ...
        // احذف كلمة المرور النصية القديمة
